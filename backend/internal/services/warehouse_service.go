...
func (s *WarehouseService) GetDashboard(storeID uint) (*dto.WarehouseDashboardResponse, error) {
	...
	if err := db.Model(&map[string]interface{}{}).Table("purchase_orders").Where("store_id = ? AND created_at >= ?", storeID, start).Count(&importOrders).Error; err != nil {
		return nil, err
	}

	var importedProducts int64
	if err := db.Table("purchase_order_items").Select("COALESCE(SUM(quantity),0)").Joins("JOIN purchase_orders ON purchase_orders.id = purchase_order_items.purchase_order_id").Where("purchase_orders.store_id = ? AND purchase_orders.created_at >= ?", storeID, start).Scan(&importedProducts).Error; err != nil {
		return nil, err
	}

	var totalAmount int64
	if err := db.Table("purchase_orders").Select("COALESCE(SUM(total_amount),0)").Where("store_id = ? AND created_at >= ?", storeID, start).Scan(&totalAmount).Error; err != nil {
		return nil, err
	}
